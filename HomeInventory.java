...
Name : Harshada Ramdas Gawade
Email: gawadeharshada7@gmail.com
...
class PhotoPanel extends JPanel
{
    public void paintComponent(Graphics g)
    {
        ...
        ImageIcon(HomeInventoryManagement.photoTextArea.getText()).getImage( );
        int w = getWidth();
        int h = getHeight();
...
public class HomeInventoryManagement extends JFrame
{
    ...
    public static void main(String[] args) {
        //create frame
        new HomeInventoryManagement().setVisible(true);
    }
...
class InventoryDocument implements Printable
{
    ...
    {
        Graphics2D g2D = (Graphics2D) g;
        if ((pageIndex + 1) > HomeInventoryManagement.lastPage)
        {
            return NO_SUCH_PAGE;
        }
        int i, iEnd;
        ...
        iEnd = HomeInventoryManagement.entriesPerPage * (pageIndex + 1);
        if (iEnd > HomeInventoryManagement.numberEntries)
            iEnd = HomeInventoryManagement.numberEntries;

        for (i = 0 + HomeInventoryManagement.entriesPerPage * pageIndex; i < iEnd; i++)
        {
            // dividing line
            ...
            g2D.drawString(HomeInventoryManagement.myInventory[i].description, (int) pf.getImageableX(), y);
            y += dy;
            g2D.setFont(new Font("Arial", Font.PLAIN, 12));
            g2D.drawString("Location: " + HomeInventoryManagement.myInventory[i].location, (int)(pf.getImageableX() + 25), y);
            y += dy;
            if (HomeInventoryManagement.myInventory[i].marked)
            ...
            y += dy;
            g2D.drawString("Serial Number: " + HomeInventoryManagement.myInventory[i].serialNumber, (int) (pf.getImageableX() + 25), y);
            y += dy;
            g2D.drawString("Price: $" + HomeInventoryManagement.myInventory[i].purchasePrice + ",Purchased on: " + HomeInventoryManagement.myInventory[i].purchaseDate, (int) (pf.getImageableX() + 25), y);
            y += dy;
            g2D.drawString("Purchased at: " + HomeInventoryManagement.myInventory[i].purchaseLocation, (int) (pf.getImageableX() + 25), y);
            y += dy;
            g2D.drawString("Note: " + HomeInventoryManagement.myInventory[i].note, (int) (pf.getImageableX() + 25), y);
            y += dy;
            try
            {
                // maintain original width/height ratio
                Image inventoryImage = new ImageIcon(HomeInventoryManagement.myInventory[i].photoFile).getImage ();
